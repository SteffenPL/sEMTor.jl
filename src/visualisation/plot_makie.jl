...
function plot_state!(f, state, p, rect = minimal_plot_rect(state, p); 
    s_nd = Node(state), title = p.sim.name * "\n", 
    ...
    limits!(ax, PSE.get_xlims(rect), PSE.get_ylims(rect))
    ...
    scatter!(ax, nuclei_pos,
        markersize = R_soft, markerspace=SceneSpace,
    ...
    if show_age
        ...
        for p in LinRange(0.3, 1.0, 10)
            scatter!(ax, nuclei_pos,
            markersize = @lift( p .* $R_soft ), markerspace=SceneSpace,
            color = cell_age_rel, colormap=cm_age, #cm_age, #(:white,0.4), #colormap = cm=cm_age,
    ...
    if hard_col
        scatter!(ax, nuclei_pos,
                    markersize = R_hard, markerspace=SceneSpace,
    ...
    scatter!(ax, apical_pos, marker = ap_m, markersize = ms, markerspace=SceneSpace, color = :red)
    scatter!(ax, basal_pos, marker = ba_m, markersize = ms, markerspace=SceneSpace, color = :black)
